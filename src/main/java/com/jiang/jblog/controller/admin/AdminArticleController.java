...
import com.jiang.jblog.service.ArticleService;
...
import com.jiang.jblog.util.ResultInfoFactory;
...
/**
 * 后台管理 文章controller
 * FILE: com.jiang.jblog.controller.admin.AdminArticleController.java
 * AUTHOR: jiang
 * DATE: 2017/4/15
 * TIME: 22:00
 */
...
public class AdminArticleController {
    ...
    @ResponseBody
    public com.jiang.jblog.vo.Pager initPage(com.jiang.jblog.vo.Pager pager) {
     ...
     */
    @RequestMapping("/load")
    public String loadArticle(com.jiang.jblog.vo.Pager pager, Integer categoryId, String tagIds, String title, Model model) {
        /**
         * 设置start位置
         */
        ...
        //获取文章列表
        List<com.jiang.jblog.vo.Article> articleList = articleService.loadArticle(param);
    ...
    @RequestMapping("/save")
    @ResponseBody
    public ResultInfo saveArticle(com.jiang.jblog.vo.Article article, int[] tags){
        try {
    ...
    public String updatePage(Integer id,Model model){
        com.jiang.jblog.vo.Article article = articleService.getArticleById(id);
    ...
    public String updateInfo(Integer articleId,Model model){
        com.jiang.jblog.vo.Article article = articleService.getArticleById(articleId);
        List<Category> categoryList = categoryService.getCategoryList();
        List<Tag> tagList = tagService.getTagList();
        model.addAttribute("article",article);
        model.addAttribute("categoryList",categoryList);
    ...
    @ResponseBody
    public ResultInfo updateArticle(com.jiang.jblog.vo.Article article, int[] tags){
        try {
            //解码文章内容防止出现部分特殊字符串被转义
            article.setDescription(URLDecoder.decode(article.getDescription(),"UTF-8"));
            article.setTitle(URLDecoder.decode(article.getTitle(),"UTF-8"));
